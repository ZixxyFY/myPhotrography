...
const db = {
  ...
  projects: {
    contracts: [
      { id: 1, name: "Wedding Photography Agreement", status: "Requires Signature" }
    ],
    shotList: [
      { id: 1, task: "First Look (Bride & Groom)", done: true },
      { id: 2, task: "Family Portraits (Bride's Side)", done: false },
      { id: 3, task: "Detail shot of the rings", done: false },
    ]
  }
};
            ...
            <div className="row g-4">
              <div className="col-md-6">
                <div className={glassClass} style={styles.glass}>
                  <h5 className="text-white mb-4 fw-bold" style={styles.fontHeader}><FileText className="me-2 mb-1" size={20} color={styles.gold}/> Contracts & Documents</h5>
                  {db.projects.contracts.map(contract => (
                    <div key={contract.id} className="p-3 rounded-3 mb-3 d-flex justify-content-between align-items-center" style={{backgroundColor: 'rgba(0,0,0,0.3)', border: '1px solid rgba(197,160,89,0.3)'}}>
                      <div>
                        <h6 className="text-white mb-1" style={{fontSize: '0.9rem'}}>{contract.name}</h6>
                        <span style={{color: '#dc3545', fontSize: '0.75rem', fontWeight: 'bold'}}><i className="fas fa-circle me-1" style={{fontSize: '8px'}}></i>{contract.status}</span>
                      </div>
                      <button className="btn btn-sm rounded-pill fw-bold px-3" style={{backgroundColor: styles.gold, color: '#1A1A1B'}}>Review</button>
                    </div>
                  ))}
                </div>
              </div>
              <div className="col-md-6">
                <div className={glassClass} style={styles.glass}>
                  <h5 className="text-white mb-4 fw-bold" style={styles.fontHeader}><CheckCircle className="me-2 mb-1" size={20} color={styles.gold}/> Shot List Builder</h5>
                  <div className="d-flex flex-column gap-2 mb-4">
                    {db.projects.shotList.map(shot => (
                      <label key={shot.id} className="d-flex align-items-center p-3 rounded-3 cursor-pointer" style={{backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)'}}>
                        <input type="checkbox" className="form-check-input me-3" defaultChecked={shot.done} style={{borderColor: styles.gold, backgroundColor: shot.done ? styles.gold : 'transparent'}} />
                        <span style={{color: shot.done ? '#A0A0A0' : '#F5F5F7', textDecoration: shot.done ? 'line-through' : 'none'}}>{shot.task}</span>
                      </label>
                    ))}
                  </div>
                  <button className="btn w-100 rounded-pill fw-bold" style={{border: '1px dashed rgba(255,255,255,0.3)', color: '#A0A0A0'}}>+ Add New Request</button>
                </div>
              </div>
            </div>
          </div>
        ...
        return (
          <div className="fade-in animate-fade-in">
            <div className="d-flex justify-content-between align-items-center mb-4">
              <h2 className="fw-bold text-white mb-0" style={styles.fontHeader}>Galleries & Prints</h2>
              <button className="btn rounded-pill fw-bold px-4" style={{backgroundColor: 'rgba(197, 160, 89, 0.1)', color: styles.gold, border: `1px solid ${styles.gold}`}}>Print Store</button>
            </div>
            <div className="row g-4">
              {db.deliveredGalleries.map(gallery => (
                <div className="col-md-6 col-lg-4" key={gallery.id}>
                  {/* FIX: Removed card class and forced dark background */}
                  <div className="border-0 shadow-sm overflow-hidden rounded-4 h-100" style={{backgroundColor: '#242426', border: '1px solid rgba(255,255,255,0.08)'}}>
                    <img src={gallery.thumb} alt={gallery.name} style={{height: '220px', objectFit: 'cover', width: '100%'}} />
                    <div className="p-4">
                      <h5 className="fw-bold text-white mb-1">{gallery.name}</h5>
                      <small className="d-block mb-4" style={{color: '#A0A0A0'}}>{gallery.date}</small>
                      
                      <div className="mb-4">
                        <div className="d-flex justify-content-between mb-2" style={{fontSize: '0.75rem', fontWeight: 'bold', color: '#A0A0A0'}}>
                          <span>Album Selection</span>
                          <span>{gallery.selected} / {gallery.total}</span>
                        </div>
                        <div className="progress" style={{height: '6px', backgroundColor: 'rgba(255,255,255,0.1)'}}>
                          <div className="progress-bar" style={{width: `${(gallery.selected/gallery.total)*100}%`, backgroundColor: gallery.selected === gallery.total ? '#198754' : styles.gold}}></div>
                        </div>
                      </div>
                      
                      <button className="btn w-100 rounded-pill fw-bold d-flex align-items-center justify-content-center gap-2" style={{backgroundColor: styles.gold, color: '#1A1A1B'}}>
                        <Download size={16} /> Enter Gallery
                      </button>
                    </div>
                  </div>
                </div>
